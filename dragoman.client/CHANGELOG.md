...
- Create Angular project with ng: `ng new dragoman.client --defaults --skip-install --skip-git --no-standalone `.
...
- Create project file (`dragoman.client.esproj`).
- Create `launch.json` to enable debugging.
- Create `nuget.config` to specify location of the JavaScript Project System SDK (which is used in the first line in `dragoman.client.esproj`).
